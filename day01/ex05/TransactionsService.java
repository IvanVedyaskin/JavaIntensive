...
public class TransactionsService {
    ...
    static class IllegalBalance extends Exception{
    ...
    public void addUser(String name, double balance) throws IllegalBalance{
        if (balance < 0){
            throw new IllegalBalance("Balance can not is negative!");
        }
    ...
    public void startTransaction(int id1, int id2, double sum) throws IllegalArgumentException{
        ...
        {
            if (sum > 0 && uList.getUserById(id1).getBalance() >= sum) {
                uList.getUserById(id1).getTransaction().addTransaction
                        (new Transaction(tmp, uList.getUserById(id1), uList.getUserById(id2), -sum));
                uList.getUserById(id2).getTransaction().addTransaction
                        (new Transaction(tmp, uList.getUserById(id2), uList.getUserById(id1), sum));
            }
            else {
    ...
    public Transaction removeTransaction(int idU, UUID idT){
        return uList.getUserById(idU).removeTransaction(idT);
    }
